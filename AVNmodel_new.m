function [dY, dati] = AVNmodel_new(time, Y, bpm)
%AVN model
% time (millisecond)
% current in pA/pF
% concentration in mM
...
beta_d_T  = 1.068*exp(-(V+26.3+cat_f_shift)/30); %1/ms
...
%% Membrane potential
dY(1, 1) = ( -(i_Na+i_Nas+i_Ca_L+i_Ca_D+i_Ca_T+i_K1+i_K_r+i_to+i_sus+ i_f+ i_K_s + i_b_Na+i_b_K+i_b_Ca+i_p+i_NaCa+ i_st+ i_SK+ i_K_ACh )) +i_stim;
...
ISK   = i_SK;
IKACh = i_K_ACh;
dati  = [INa, INas, ICaL , ICaD, ICaT, Ikr,Ik1, Ito, Isus, If, IbNa, IbK, IbCa, Ip, INaCa, Ist, IKs, h1, h2,m, d_D, f_D, d_T,f_T, Istim, d_s, f_s, m_inf, tau_m,h1_inf, tau_h1, tau_h2, d_D_inf, f_D_inf, tau_d_D,tau_f_D, d_T_inf, f_T_inf, tau_d_T,tau_f_T, ms, hs1, hs2, ISK, m_cak , j_Ca_dif, j_up, j_tr, j_SRCarel, IKACh, Cai,Nai, Ki, Ca_sub, CaNSR, CaJSR,O1, R1, I1, tau_P, xs, h, hs,r,q,P];
end